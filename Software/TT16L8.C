...
//
//  NOTE:  This requires 32-bit C and assumes ints to be 32 bits.
//
...
#define INPUT_BITS 16                   // number of input bits
#define DEDICATED_INPUT_BITS 10         // input bits that are immutable
#define DEDICATED_INPUTS 0x3ff          // which inputs are dedicated as a mask
#define OUTPUT_BITS 8                   // number of output bits
#define DEDICATED_OUTPUTS 0x81          // dedicated output bits
#define PAL_SIZE (1L<<INPUT_BITS)        // 2**number of inputs
#define PAL_NAME "PAL16L8"              // What is this thing
...
char
  ProgramName[_MAX_FNAME],              // the actual name of the program
  BareFile[_MAX_FNAME];                 // The name of our sample file, sans
                                        // path or extension.
...
//  Names of pins on the PAL16L8.

  char  *inPins[] =
  {
    "Pin1", "Pin2", "Pin3", "Pin4", "Pin5", "Pin6", "Pin7", "Pin8", "Pin9",
    "Pin11", "Pin18", "Pin17", "Pin16", "Pin15", "Pin14", "Pin13"
  };
...
//  Sign on.
  ...
  printf( "\n %s - Read PAL16L8 map file and produce table.\n\n",
    ProgramName);
...
//  Next, figure which of the 6 programmable lines are outputs.
...
//  Show the outputs and inputs.
  ...
  printf( "\nBefore optimization:\n"
    "  Input mask = %04x (%d bits); Output mask = %02x(%d bits)\n",
...
;  printf( "Old output mask = %02x, New output mask = %02x\n", OutMask, sigMask);
  OutMask &= sigMask;
  return;
} // FindDontCareBits
